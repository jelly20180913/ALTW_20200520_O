...
namespace WebApi.Common
{
    ...
    public class XSLXHelper
    {
        ...
        /// <param name="mode">true:DescriptionAttribute false:Attribute</param>
        /// <returns></returns>
        public XLWorkbook Export<T>(List<T> data,bool mode)
        {
            //建立 excel 物件
            ...
            foreach (var item in typeof(T).GetProperties())
            {
                ...
                else sheet.Cell(1, colIdx++).Value = item.Name; 
                #endregion
                #region - 直接使用物件屬性名稱 -
                //或是直接使用物件屬性名稱
